...
import { P as PostContext, g as getBlogUrl, s as slugMap } from '../chunks/chunk-CczHCglJ.js';
...
/*! /home/runner/work/craigory-dev/craigory-dev/libs/blog-posts/src/lib/components/package-manager-tabs.module.scss [vike:pluginModuleBanner] */
...
/*! /home/runner/work/craigory-dev/craigory-dev/libs/blog-posts/src/lib/components/package-manager-tabs.tsx [vike:pluginModuleBanner] */
...
function isManager(value) {
  return typeof value === "string" && MANAGERS.includes(value);
}
function usePackageManagerSelection() {
  const [manager, setManager] = reactExports.useState(DEFAULT_MANAGER);
...
function PackageManagerTab({ children }) {
...
function PackageManagerTabs({ children }) {
  ...
  const panels = reactExports.Children.toArray(children).filter(
    (child) => reactExports.isValidElement(child) && child.type === PackageManagerTab
  );
  const availableManagers = panels.map((panel) => panel.props.manager);
  const active = availableManagers.includes(selected) ? selected : availableManagers[0] ?? DEFAULT_MANAGER;
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: classes$1["tabs"], children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "div",
      {
        className: classes$1["tabList"],
        role: "tablist",
        "aria-label": "Package manager",
        children: MANAGERS.filter((manager) => availableManagers.includes(manager)).map(
          (manager) => {
