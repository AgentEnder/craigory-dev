...
describe('playlistToCsv', () => {
  it('emits a header and one row per track', () => {
    ...
    expect(rows[0]).toBe('Title,Artist,Album,ISRC,Release Date,Spotify,Apple Music,YouTube,Deezer,Bandcamp,Last.fm,Pandora');
  ...
  it('handles an empty playlist', () => {
    expect(playlistToCsv(playlist([]))).toBe(
      'Title,Artist,Album,ISRC,Release Date,Spotify,Apple Music,YouTube,Deezer,Bandcamp,Last.fm,Pandora\r\n'
    );
  });
});
