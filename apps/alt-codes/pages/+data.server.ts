...
import { CP437_SPECIAL, type CharacterEntry } from '../src/unicode-data';

export type Data = {
  characters: CharacterEntry[];
};

// Build CP437 bidirectional maps
const altToUnicode = new Map<number, number>();
for (const [altCode, cp] of CP437_SPECIAL) {
  altToUnicode.set(altCode, cp);
}
for (let i = 32; i <= 126; i++) {
  altToUnicode.set(i, i);
}
const unicodeToAlt = new Map<number, number>();
for (const [altCode, cp] of altToUnicode.entries()) {
  unicodeToAlt.set(cp, altCode);
}
...
function rangeEntries(start: number, end: number, categoryId: string): CharacterEntry[] {
  ...
  for (let cp = start; cp <= end; cp++) {
    out.push(makeEntry(cp, categoryId));
  }
  return out;
}

export async function data(): Promise<Data> {
  // Alt codes section: ordered by alt code number (1–254)
  const altCodeEntries: CharacterEntry[] = [];
  for (let altCode = 1; altCode <= 254; altCode++) {
    const cp = altToUnicode.get(altCode);
    if (cp !== undefined) {
      altCodeEntries.push(makeEntry(cp, 'alt-codes'));
    }
  }
