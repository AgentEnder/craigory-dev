...
async function runMigrationsWithRecovery(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    spinner.message('Triaging migration failure...');
...
export async function runNxMigrate(
  ...
  const s = p.spinner();
  s.start(
    `Nx migration: v${currentMajor} → v${latestMajor} (latest: ${latest})`
  );
  ...
  while (currentMajor <= latestMajor) {
    ...
    // Phase 1: nx migrate <target>
    s.message(`nx migrate ${targetVersion}...`);
    ...
    if (migrateResult.exitCode !== 0) {
      s.message(`nx migrate ${targetVersion} failed — triaging...`);
      ...
      if (triage.action === 'halt') {
        s.stop(`Nx migration halted: ${triage.reason}`);
        return null;
      }
    }
    ...
    if (!stats.ok) {
      s.stop(`Nx migration to v${currentMajor} failed`);
      return null;
    }

    // Report this major step
    if (stats.totalMigrations > 0) {
      s.message(
        `nx@${targetVersion}: ${stats.totalMigrations} migrations, ${stats.migrationsWithChanges} changed`
      );
    }
  ...
  s.message('Processing AI migration files...');
  s.stop(`Nx: ${oldVersion} → ${getNxVersion(repoPath) ?? 'unknown'}`);
