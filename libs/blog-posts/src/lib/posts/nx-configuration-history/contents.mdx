...
Additionally, we took some time to reevaluate where existing configuration options lived. Since we now have a configuration file per project it made less sense to place `tags` and `implicitDependencies` in `nx.json`. We moved these properties to `nx.json` as well.
